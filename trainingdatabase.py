...
def read_peak(filename, volume, kappa_spec, correction, led_used, led_used_df, device=1., additional=False, blank=None,
    ...
    [df_, header, unit] = read_rawdata(filename, led_used=led_used, additional=additional, blank_corr=blank_corr,
    ...
    df = pd.DataFrame(np.zeros(shape=(len(df_.index), 0)), index=df_.index)
    for i in df_.columns:
        if led_used_df.ix[0, i] == True:
            df.ix[:, i] = df_.ix[:, i]
    ...
    mean_.set_index = df_.name
    mean_.name = df_.name

    return mean_


def read_directory(dirname, volume, kappa_spec, led_used, led_used_df, device=1., correction=False, additional=False,
    ...
    l1 = [read_peak(i, led_used=led_used, led_used_df=led_used_df, volume=volume, kappa_spec=kappa_spec,
                    correction=correction, device=device, additional=additional, blank_corr=blank_corr,
                    blank_mean_ex=blank_mean_ex, blank_std_ex=blank_std_ex, normalize=normalize,
...
def detection(header, rg665, rg9, kappa_spec, date, current, unit_blank, led_used, full_calibration=False,
    ...
    [mean_ex_corr, std_ex_corr, unit_blank] = mean_conversion(led_order=led_order, led_all=led_all, rg665=rg665,
                                                              rg9=rg9, kappa_spec=kappa_spec, date=date,
                                                              current=current, unit_blank=unit_blank,
                                                              full_calibration=full_calibration,  blank_mean=blank_mean,
                                                              blank_std=blank_std, correction=correction, device=device)
...
def mean_conversion(led_order, led_all, rg665, rg9, kappa_spec, date, current, unit_blank, full_calibration=False,
    ...
    if correction is True:
        ...
        blank_em_bal = emission_correction_table(blank_std, device, rg9, rg665, led_order=led_all)
...
def correction_led(mean, kappa_spec, date, current, peakcolumns, full_calibration=True, print_info=True):
    ...
    if not current:
        if full_calibration is True:
            current = [97]*len(peakcolumns)
        else:
            current = [50]*len(peakcolumns)
...
def construction_training_database(data, trainingsdata, phytoplankton, xcoords, led_used_, device=1, device_training=0.,
                                   ...
                                   correction=True,  additional=False, peak_detection=True, blank_corr=True,
                                   blank_mean_ex=None, blank_std_ex=None, plot_raw=False, type_=2,
                                   full_calibration=True, normalize=True, additional_training=False, standardize=True,
                                   save=False):
...
# ----------------------------------------------------------------------------------------------------
    ...
    [training, training_current_, RG9_training, RG665_training,
     training_date] = read_directory(trainingsdata, volume, led_used=led_used, led_used_df=led_used_, kappa_spec=None,
                                     device=device_training, additional=additional_training, blank_corr=blank_corr,
                                     blank_mean_ex=None, blank_std_ex=None, normalize=False, standardize=False,
                                     full_calibration=full_calibration)

    # reduce training matrix to the number of LEDs used
    training_red = pd.DataFrame(np.zeros(shape=(len(training.index), 0)), index=training.index)
    training_current = pd.DataFrame(np.zeros(shape=(len(training_current_.index), 0)), index=training_current_.index)
    for i in training.columns:
        if led_used_.ix[0, i] == True:
            training_red.ix[:, i] = training.ix[:, i]
            training_current.ix[:, i] = training_current_.ix[:, i]
    ...
    if correction is True:
        ...
        mean_corr = correction_led(mean=mean_balanced, kappa_spec=kappa_spec, date=date, current=current,
                                   peakcolumns=peak.columns, full_calibration=full_calibration, print_info=True)
        ...
        for s in range(len(training_balanced.index)):
            ...
            training_corr_trans = correction_led(mean=mean_tra, kappa_spec=kappa_spec,
                                                 date=training_date.ix[s, :].values[0], current=cur_training,
                                                 peakcolumns=training_current.columns,
...
def gen_train_data():
    # Loading of data -------------------------------------------------------
    # folder with training objects
    training_db = 'supplementary/trainingdatabase/trainingobjects/2016_Bilbao_50mA_100mikroE/'
    # direction to phytoplankton information
    ...
    # Default system parameter; not relevant for the result but required to process the algorithm
    ...
    pigment_pattern] = construction_training_database(df_, training_db, phytoplankton=phytoplankton,
                                                      led_used_=led_used_, xcoords=xcoords_sample, device=1,
                                                      device_training=device, kappa_spec=kappa_spec,
                                                      separation=separation_, priority=priority, pumprate=pumprate,
                                                      correction=correction, blank_corr=blank_correction,
                                                      full_calibration=full_calibration, normalize=normalization,
                                                      standardize=standardization, save=save)
